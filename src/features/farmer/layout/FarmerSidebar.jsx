  ...
  return <>
    ...
    <aside id="farmer-sidebar" aria-label="Farmer sidebar" className={`fixed left-0 top-0 z-50 flex h-dvh w-[238px] flex-col overflow-hidden border-r border-slate-200 bg-white px-3.5 py-5 transition-transform duration-300 ${open ? 'visible translate-x-0' : 'invisible -translate-x-full lg:visible lg:translate-x-0'}`}>
      ...
      <nav className="farmer-sidebar-nav space-y-1" aria-label="Farmer navigation">{farmerNavigation.map(({ label, to, icon: Icon, badge }) => {
        ...
        return to === routes.farmer.dashboard || to === routes.farmer.farm || to === routes.farmer.products || to === routes.farmer.orders || to === routes.farmer.cropDoctor || to === routes.farmer.priceInsights || to === routes.farmer.agriStores
          ? <NavLink key={to} to={to} end={to !== routes.farmer.products} onClick={onClose} className={({ isActive }) => `${styles} ${isActive ? 'bg-emerald-100 text-emerald-800 before:absolute before:-left-3.5 before:h-8 before:w-1 before:rounded-r before:bg-emerald-700' : 'text-slate-700 hover:bg-slate-50'}`}>{content}</NavLink>
          : <span key={to} aria-disabled="true" className={`${styles} text-slate-700`}>{content}</span>
      })}</nav>
      <div className="farmer-sidebar-promo relative mt-auto hidden h-[385px] shrink-0 overflow-hidden rounded-xl bg-gradient-to-b from-emerald-50 to-emerald-100 p-4 xl:block"><Leaf className="mb-2 size-10 fill-emerald-400 text-emerald-700" /><p className="relative z-10 text-xl font-black leading-[1.05] text-emerald-950">Better<br />Farmers<br />Brighter<br />Futures</p><p className="relative z-10 mt-3 max-w-32 text-xs leading-snug text-slate-700">Technology for healthier farms and happier communities.</p><div className="absolute inset-x-0 bottom-0 h-[150px] bg-[url('/images/dashboard/farmer-hero.png')] bg-cover bg-center" /></div><p className="farmer-sidebar-version pt-3 text-center text-[11px] text-slate-500">v1.0.0</p>
    </aside>
  </>
}
